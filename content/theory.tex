...
ozone on \ch{NO_x}. Next, I will describe the fluid dynamics of the
...
\ch{NO} to \ch{NO2} converter. I will introduce the rate and the
...
With this motivation in mind, a measure for the reaction speed is
...
conserve ozone concentration or it can react to form \ch{O2} by the
following mechanism
...
monoxide (\ch{NO}, also referred to as nitric oxide) to nitrogen
...
that the oxidation of \ch{NO2} is compensated. 

Unfortunately, this is not the only reaction using \ch{NO3}. It can
react with \ch{NO2} to generate the solid \ch{N2O5}
...
Another effect that has to be considered is the adsorption of the
...
equilibrium has been reached. The \ch{NO} concentration is normally on
...
\ch{NO2}. Here the evaporation of ozone could lead to longer purge
...
The next requirement a nigh \SI{100}{\%} conversion of \ch{NO}
to \ch{NO2} is necessary. This leads to three considerations. First,
...
Concerning the mixing with ozone, Section~\ref{sec:fluid} supplies an
...
about a \SI{2}{\%} error in the retrieved \ch{NO} concentration should
be expected. Hopefully, the results will be better than that, as some
of the produced \ch{NO3} will (as described in Section~\ref{sec:o-no})
quickly degrade to \ch{NO2} and the ozone concentration is likely to
be underestimated.

As a last step I the length of the reaction tube is calculated from
...
algorithm can befound in Figure~\ref{fig:icad}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{ICAD.png}
