...
and I would like to suppress them as much as possible. However, there
...
of change of compound concentration. In this section we will use $c$
to denote concentrations in \si{\per\cubic\centi\meter}. Using the
above mentioned prototypic equation, we can see that the change of
concentration, i.\,e. $\dot c$, of the different species are related
by:
\begin{align*}
...
In the case of the conversion the ozone concentration will normally far
exceed the concentration of the other trace gases. This allows one to
think of its concentration as approximately constant. If $c(B)$ is set
constant in Equation~\eqref{eq:rate}, then this yields a well known
ordinary first order differential equation for $c(A)$ with the solution
\begin{align*}
...
dependence, it is difficult to give a closed formula. Especially I
...
wavelength limit for an ignition if the reaction is stated.
...
atmosphere.  It is often referred to as \emph{Chapman-Cycle}
due to its first investigator (c.\,f.~\cite{chapman}
and~\cite{roedel}). As it turns out, one can use these atmospheric
...
case we use a mercury lamp, which also has a strong peak at
...
such that in absolute terms we will still produce an ozone
surplus. The expected ozone value can be computed exactly using the
...
different currents, but since the exact value is of no special
interested and a magnitude suffices, this estimate will be enough for
this purpose. The experimentally verified output of the built generator
can be found in Section~\ref{sec:ozone}.
...
would be achieved. However, this is not the only reaction taking
...
quickly. If we assume no \ch{NO3} in the sample air (which is sensible
...
concentrations are small. In tnhe further analysis we will assume that
...
nitrogen monoxide will lead to additional nitrogen dioxide, which will
in turn distort the measurement results. As will be seen in
Section~\ref{sec:adsorption}, this problem cannot be solved by
filtering \ch{N2O} as it passes most filter barriers, so the reaction
products have to be removed after the transition took place.
...
achieved. These questions can all be answered using a few basic
principles from fluid dynamics. First I will have a look at laminar
flows in cylindric vessels (in this case tubes) to get a connection
between the flow and the maximum speed in the tube. Next I will have a
look at diffusion of gases. As there will only be laminar flows,
diffusion is the only way mixing can take place, so an estimate for
the time this mixing takes is required. Lastly I will discuss
adsorption of some trace gases at teflon, silica gel and activated
carbon to better the understanding of the filtering effects in the
instrument.

\subsubsection{Laminar flows in cylinders}
\label{sec:cylinder}

I am interested in what regime the gas transport in the tube
...
system as it would ensure a faster mixing of the gases. To see whether
one can expect a laminar or a turbulent flow, we use the
phenomenological \emph{Reynold's number}
...
Denote by $r_0$ the inner radius of the tube and by $l$ its length. We
assume that we are in the equilibrium state, such that all forces need
to add to zero. Consider a cylinder of radius $0 \leq r \leq
...
to be at an equilibrium, thus $F_b = F_f$ needs to hold for all
...
computation the dwell time of the gases.
...
can occur is \emph{diffusion}. Therefore I would like an estimate for
the time it takes for diffusion to take place. This exposition drew
...
\SI{1}{\per\square\meter\second}) of the species is proprotional to
the concentration gradient $\nabla n$ and that the species flows from
higher to lower concentration. We yield
...
\end{align*}
where for the last equation an istotrope medium, i.\,e.\
...
of species at small time scales is most interesting, we can assume the
vessel inifinitely large. With this the equation becomes accessible
...
\end{align}
To determine $c(k)$, an initial value is required. Again an
...
computation the inverse Fourier transform of $\hat n$ has to be
performed. However, it is a well known fact that the Gauß function is
an Eigenfunction of the Fourier transform (and its
invers). Additionally the total concentration needs to be $n_0$ for
all times $t$, which gives its normalization. Thus the solution has
to take the form
...
undergoing diffusion. Setting $n_0 = 1$, we arrive at the probability
...
the \emph{mean squared displacement}, we yield the following relation
...
zero air has to be free of all trace gases, which are to be
measured. Experience shows that a filter chain of activated carbon,
...
the ozone generator was explored. This poses the difficulty of
filtering it without removing the ozone, too. In~\cite{ozone-silica}
it is mentioned that silica gel can be used as an ozone storage
...
\ch{NO} concentration is normally on the order of magnitude of a few
\SI{10}{ppb} perhaps a few \SI{100}{ppb} if we consider downtown
stationary measurements. So for all reasonable adsorption capacities
there is always enough ozone left for the full conversion. The only
interesting instance is during a switch where the ozone stream is
turned off to only measure \ch{NO2}. Here the evaporation of ozone
could lead to longer purge times and hence to a worse time
resolution. The behaviour of the system after such switches has been
researched in Section~\ref{sec:switch}.
...
normal procedure consists of using an activated charcoal, a silica gel
and a particle filter. To compensate for the laughing gas mentioned in
...
for considerable loss in \ch{NO2} due to further oxidation.

Concering the mixing with ozone, Section~\ref{sec:fluid} supplies an
estimate the mixing time. As there are mainly laminar flows in the
...
case we use tubes with $d = \SI{4}{\milli\meter}$ and the diffusion
...
be adjusted. For this we have to set a desired conversion ratio. For
these estimates we work with a ratio of \SI{99}{\%}. Since the ozone
generator will supply an overabundance of \ch{O3}, the
linear ordinary differential equation approximation introduced in
Section~\ref{sec:rate} is applicable. Thus the conversion ratio is given by
\begin{align*}
  \exp(-kc(\ch{O3})\cdot t),
\end{align*}
...
Together with Equation~\eqref{eq:react-bound} this one yields
\begin{align*}
...
Mie scattering. Next, a better understanding of the influence of the
trace gases on $\epsilon_a$ is necessary. The absorption should be
proportional to the concentration of the gases, so one writes
...
$L$. If one next want to compute the concentration of the trace gases,
onle the absorption cross sections are required, which can be found in the
...
results so far into the Lambert-Beer equation lead to
...
instrument cannot b detect. The effect of this lower resolution is
that narrow emission lines get smeared out. This smearing out is
called \emph{instrument function}. To determine it, one can measure the
...
having a managable geometrical pathlength. The price one pays by
taking this approach lies in the fact that then the pathlength becomes
...
Furthermore, there is the transmittance $T_g$ of the gas in the
...
intesity $I_{\text{out}}$ can be computed using a geometric series
...
still be obainable by comparing $I$ and $I_0$. Again an exponential
...
the results for the intensities $I$ and $I_0$ are plugged into
Equation~\eqref{eq:opt-dens} 
...
To evaluate this expression further, a
...
\end{align}
With this definition of $L_0$ it is completely independent of any
...
of the setup (if we assume $\epsilon_0$ to be fixed) and desired
